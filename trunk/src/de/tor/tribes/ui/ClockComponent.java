...
class ClockComponent extends JComponent {
    ...
    private void drawClockFace(Graphics2D g2) {
        ...
        for (int sec = 0; sec < 60; sec++) {
            ...
            if (sec % 5 == 0) {
                ...
                drawValue(g2, sec / 60.0, tickStart - 10, radius, sec/5);
            } else {
    ...
    private void drawValue(Graphics2D g2, double percent,
            int minRadius, int maxRadius, int val) {
        double radians = (0.5 - percent) * TWO_PI;
        double sine = Math.sin(radians);
        double cosine = Math.cos(radians);
        ...
        int dxmax = _centerX + (int) (maxRadius * sine);
        int dymax = _centerY + (int) (maxRadius * cosine);
        g2.drawString("" + val, dxmin, dymin);
    }

    public static void main(String[] args) {
        final JFrame f = new JFrame();
        final ClockComponent cc = new ClockComponent();
        cc.start();
        f.add(cc);
        f.setSize(300, 300);
        f.pack();
