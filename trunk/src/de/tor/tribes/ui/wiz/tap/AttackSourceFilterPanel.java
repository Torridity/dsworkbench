...
public class AttackSourceFilterPanel extends WizardPage {
    ...
    private void filterTroops(List<TAPAttackSourceElement> pAllElements) {
        ...
        int requiredTroopAmount = UIHelper.parseIntFromField(jMinFarmSpace);
        int requiredTroopAmountBonus = UIHelper.parseIntFromField(jMinFarmSpaceBonus);
        if (requiredTroopAmount > 0) {
            for (TAPAttackSourceElement element : pAllElements) {
                ...
                if (troopsForVillage != null) {
                    if (element.getVillage().getType() == Village.FARM_BONUS) {
